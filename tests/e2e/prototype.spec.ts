...
test('Pass 2 journey reaches Vendor readiness while admin documents remain later', async ({ page }, testInfo) => {
  ...
  await expect(religionException.locator('.product-search-results button')).toHaveCount(1)
  await expect(religionException.getByText('BK-PPKN-5', { exact: true })).toHaveCount(0)
  await expect(religionException.getByText('BK-SENI-5', { exact: true })).toHaveCount(0)
  await expect(religionException.getByText('BK-ENG-5', { exact: true })).toHaveCount(0)
  const productSearch = religionException.getByLabel('Cari Product Master')
  await productSearch.fill('Pendidikan Pancasila')
  await expect(religionException.getByRole('button', { name: /Pilih BK-PPKN-5/ })).toBeVisible()
...
test.describe('mobile operations layout', () => {
  ...
  test('renders Pass 2 workflows as stacked mobile operations', async ({ page }, testInfo) => {
    ...
    await expect(mobileReligion.locator('.product-search-results button')).toHaveCount(1)
    await expect(mobileReligion.getByText('BK-PPKN-5', { exact: true })).toHaveCount(0)
    await expect(mobileReligion.getByText('BK-SENI-5', { exact: true })).toHaveCount(0)
    await expect(mobileReligion.getByText('BK-ENG-5', { exact: true })).toHaveCount(0)
    await expect.poll(() => page.evaluate(() => document.documentElement.scrollWidth)).toBe(390)
